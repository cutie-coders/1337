...
typedef void* p_dynamic_key_t, * p_websocket_t;

typedef struct
{
	CHAR hwid[33];
	UINT8 cheat_access;
	INT64 subs_diff_time;
	SIZE_T dll_raw_size;
	PVOID p_dll_raw;
	p_websocket_t _p_websocket;
	p_dynamic_key_t _p_cli_dynamic_key, _p_serv_dynamic_key;
	PUINT8 p_cli_dynamic_key_raw;
} s_subs_info;

#define DEF_KEY_SIZE 1024
...
#define has_cheat_access(cheat_access) ((cheat_access > 0) && (cheat_access < (1 << 4)))

extern std::string format_subs_diff_time(int64_t subs_diff_time);

extern "C" void server_api_init();
extern "C" bool server_api_get_subs_info(s_subs_info*, const char*, const char*, const char*, const uint8_t*, size_t);
extern "C" bool server_api_get_cheat_dll(s_subs_info*, uint8_t);
extern "C" void server_api_ban_hwid(const char*, const char*, const uint8_t*, size_t);
extern "C" void server_api_update_loader(const char*);

extern "C" void server_api_get_patterns(const char* c_url, uint8_t * p_patterns);

__forceinline int Exit()
{
	int a = 0, b = 0;
	__asm int 0;
	return a / b;
...
#define ban_hwid() \
[]{ \
    server_api_ban_hwid(str("485fc638077af468f8d8fc09149d286c").c_str(), str("https://1337.gang:2053/subs_check?q=").c_str(), g_hwid_raw, hwid_raw_calc()); \
    Exit(); \
}()
