...
bool c_dynamictext::hovered()
{
	...
	auto size = ImGui::CalcTextSize((*(std::string*)value).c_str());
...
bool c_dynamictext::update()
{
	...
	if (c->get_active_tab_index() != this->tab && c->get_tabs().size() > 0)
		return false;
	auto c_size = ImGui::CalcTextSize((*(std::string*)value).c_str());
...
void c_dynamictext::render() {
	...
	g_Render->DrawString(pos.x, pos.y, color.manage_alpha(alpha), render::none, fonts::menu_desc, (*(std::string*)value).c_str());
}