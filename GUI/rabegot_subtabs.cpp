...
bool c_ragebot_subtabs::update()
{
	...
	*(int*)value = std::clamp(*(int*)value, 0, (int)elements.size() - 1);
	return false;
}
	...
	if (open) {
		...
		for (size_t i = 0; i < elements.size(); i++) {
			...
			g_Render->_drawList->AddImage(current_item->texture,
				base_pos, ImVec2(base_pos.x + current_item->width * icons_scale, base_pos.y + 40 * icons_scale),/*pos.x + 10 + current_item->width * icons_scale, pos.y + 41 * icons_scale),*/
				ImVec2(0.f, 0.f), ImVec2(1.f, 1.f), *(int*)value == i ?
				main_color.manage_alpha(new_alpha).u32() : 
				clr2.u32());
			...
			g_Render->DrawString(pos.x + size.x / 2, pos.y + size.y / 2 + 10, *(int*)value == i
				? main_color.manage_alpha(new_alpha - 50.f * (1.f - animations[i]))
		...
		auto new_alpha = std::clamp(alpha * (1.f - open_animation), 0.f, 255.f);
		auto clr2 = color_t(27, 31, 40, new_alpha - 15.f * animation);
		g_Render->FilledRectGradient(pos.x, pos.y, size.x, size.y,
			clr2, clr2,
			clr, clr);
		g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, size.y + 2, color_t(40, 49, 70, new_alpha));
