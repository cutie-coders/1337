...
void CKnifebot::Run()
{
	...
	auto can_shoot = []() {
		...
		return (TICKS_TO_TIME(csgo->fixed_tickbase) >= weapon->NextPrimaryAttack()
			|| TICKS_TO_TIME(csgo->fixed_tickbase) >= weapon->NextSecondaryAttack());
	};
	...
	for (auto i = 0; i <= interfaces.global_vars->maxClients; i++)
	{
		...
		if (!pEntity->IsValid())
			continue;
		Vector stomach = g_Ragebot->GetPoint(pEntity, (int)CSGOHitboxID::Stomach, pEntity->GetBoneCache().Base());
	...
	if (best_entity != nullptr)
	{
		...
		for (int i = 0; i < 2; i++) {
			...
			Vector stomach = g_Ragebot->GetPoint(record->player, (int)CSGOHitboxID::Stomach, record->bones);
			if (stomach == Vector(0, 0, 0))
				return;
			...
			if (work > 0) {
				...
				csgo->cmd->viewangles = AimAngle;
				csgo->cmd->tick_count = TIME_TO_TICKS(record->sim_time + g_Ragebot->LerpTime());
				break;
			}
		}
	}
